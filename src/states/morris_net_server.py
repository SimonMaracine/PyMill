...
class MorrisNetServer(State):
    ...
    def __init__(self, id_, control):
        ...
        self.bg_color = str_to_tuple(MorrisNetServer.get_background_color())
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.buttons[0].pressed(event.pos, event.button):
                        pause_net.run(self._control, display.window.copy(), self.host)
    ...
    def update(self):
        ...
        if self.board.game_over:
            ...
            game_over_net.run(self._control, display.window.copy(), self.board.winner, self.host)

        if self.lost_connection and self._control.state == MORRIS_NET_STATE_SERVER:
...
def run(control):
    morris_net = MorrisNetServer(MORRIS_NET_STATE_SERVER, control)
    morris_net.run()
