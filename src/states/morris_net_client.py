...
class MorrisNetClient(State):
    ...
    def __init__(self, id_, control):
        ...
        self.bg_color = str_to_tuple(MorrisNetClient.get_background_color())
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.buttons[0].pressed(event.pos, event.button):
                        pause_net.run(self._control, display.window.copy(), self.client)
    ...
    def update(self):
        ...
        if self.board.game_over:
            ...
            game_over_net.run(self._control, display.window.copy(), self.board.winner, self.client)

        if self.lost_connection and self._control.state == MORRIS_NET_STATE_CLIENT:
...
def run(control):
    morris_net = MorrisNetClient(MORRIS_NET_STATE_CLIENT, control)
    morris_net.run()
